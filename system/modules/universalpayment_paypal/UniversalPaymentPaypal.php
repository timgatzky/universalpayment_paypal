 ...
 * Class file
 */
class UniversalPaymentPaypal extends \Frontend
{
	/**
	 * Payment Methode name registered
	 * @var string
	 */
	protected $strPaymentMethodName = 'paypal';
	...
	public function validateWidget(\Widget $objWidget, $intId)
	{
		...
		// define url params
		$arrParams = array();
		$arrParams['cmd'] = '_xclick';
		$arrParams['business'] = $objWidget->up_paypalEmail;
		$arrParams['rm'] = 2;
		$arrParams['lc'] = 'DE';
		$arrParams['currency_code'] = 'EUR';
		
		// overwrite params with user settings
		if(strlen(trim($objWidget->up_paypalUrlParams)) > 0)
		{
			$objString = \String::getInstance();
			$strParams = trim($objWidget->up_paypalUrlParams);
		...
		if(isset($arrParams['return']))
		{
			// HINT check if any GET params are set before adding the unique id
			$arrParams['return'] .= '?uid='.$this->strUniqueId;
		}
		elseif(!isset($arrParams['return']))
		{
			$arrParams['return'] .= 'http://'.$this->replaceInsertTags('{{env::host}}').'/'.$this->replaceInsertTags('{{link_url::'.$objPage->id.'}}');
			$arrParams['return'] .= '?uid='.$this->strUniqueId;
		}
		...
		$strAction = 'https://www.'. ($objWidget->up_paypalDebug ? 'sandbox.':'') . 'paypal.com/cgi-bin/webscr';
